...
app = Flask(__name__)
# Configure mysql databaes
app.config["MYSQL_DATABASE_HOST"] = 'clarusway.c0jpxn16qmtk.us-east-1.rds.amazonaws.com'
...
def insert_email(name, email):
    ...
    #if user input None (null) give warning
    if name == "" or email == "":
        response = "Username or email can not be empty!"
    # if there is no same username in db, then insert teh new one
    elif not any(result):
        insert = f"""
        INSERT INTO users
        VALUES ("{name}", "{email}");
    ...
    #if there is user with the same name, then give warning
    else:
